...
namespace input {
	...
	**/
	SDL_INPUT_API void Input::HandleEvents(SDL_Event* e)
	{
	...
	**/
	SDL_INPUT_API void Input::Update()
	{
		for (std::list<Profile*>::iterator it = this->profiles.begin(); it != this->profiles.end(); it++) {
	...
	**/
	void Input::HandleEventsProfile(SDL_Event* e, Profile *p)
	{
		// Key handler
	...
	* Keep stuff updated
	**/
	void Input::UpdateProfile(Profile *p)
	{
...
specfail:
	...
	SDL_INPUT_API Binding* Profile::absolute(BindSrc src, float* destination, Callback callback, void* userdata)
	{
		Binding* b = new Binding(src, callback, userdata);
	...
	SDL_INPUT_API Binding* Profile::relative(BindSrc src, float* destination, Callback callback, void* userdata)
	{
		Binding* b = new Binding(src, callback, userdata);
	...
	SDL_INPUT_API Binding* Profile::onoff(BindSrc src, bool* destination, Callback callback, void* userdata)
	{
		Binding* b = new Binding(src, callback, userdata);
	...
	SDL_INPUT_API Binding* Profile::toggle(BindSrc src, bool* destination, Callback callback, void* userdata)
	{
		Binding* b = new Binding(src, callback, userdata);
